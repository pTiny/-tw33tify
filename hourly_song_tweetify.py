...
def random_track():
    ...
    track = random.choice(list(artist.album_tracklist(artist.albums[album]).keys()))

    return f"""
Track suggestion of the hour:

'{track}' by {artist.artist_name}
{artist.album_tracklist(artist.albums[album])[track]}
"""

post_tweet(random_track())
print("Tweet tweeted")